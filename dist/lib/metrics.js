...
export function computeMetrics(a, b, sampleRate) {
...
function downmix(channels) {
    ...
    for (let i = 0; i < len; i++)
        out[i] = (channels[0][i] + channels[1][i]) * 0.5;
    return out;
}
...
function lraApprox(signal, sr) {
    const window = Math.floor(sr * 3);
    const values = [];
    for (let i = 0; i + window < signal.length; i += window)
        values.push(lufsApprox(signal.subarray(i, i + window)));
    values.sort((a, b) => a - b);
    if (values.length === 0)
        return 0;
...
function truePeakApprox(signal) {
    let p = 0;
...
function stereoCorrelation(channels) {
...
function midSideRatio(channels) {
    if (channels.length < 2)
        return 1;
    const l = channels[0];
    const r = channels[1];
