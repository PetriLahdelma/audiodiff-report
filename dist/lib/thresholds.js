function parseThresholds(expr) {
  return expr.split(',').map((part) => {
    const match = part.match(/([a-zA-Z]+)\s*(>=|<=|=|>|<)\s*([\w.\-]+)/);
    if (!match) throw new Error(`Invalid threshold: ${part}`);
    return { key: match[1], op: match[2], value: match[3] };
  });
}

function evalThresholds(report, thresholds) {
...
export { parseThresholds, evalThresholds };
